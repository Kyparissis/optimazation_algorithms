%   This function implements the Levenberg-Marquardt optimization 
% method to optimize multivariable functions without conditions.
%% INPUT:
% ------
...
%%
function [FinalPoints, maxIterations, x_values, y_values] = LevenbergMarquardt_method(f, InitialPoints, epsilon, stepSwitchText, stepSwitchChoise, stepValue)
    ...
    while norm(J) >= epsilon
        ...
        % Increasing u by one until the Hessian matrix is positive difinite
        % =======================
        u = 0;
        posDefHessian_f = H + u * eye(2);
        while( all(eig(posDefHessian_f) > 0) == 0 )
            u = u + 0.2;
            posDefHessian_f = H + u * eye(2);
        end

        % Solving the system to find the direction:
        % =======================
        d = -inv(posDefHessian_f) * J;
        ...
        if (strcmp(stepSwitchText,'StepChoise') && strcmp(stepSwitchChoise,'FuncMinimize'))
            ...
            resultsArr = bisection_withDerivatives_method(func2minimize, [0.001 0.3], 0.001);
        ...
        % Update gradient and hessian matrixes to  ∇f(x(k),y(k)) and
        % (∇^2)f(x(k),y(k)) accordingly
        % =======================
        J = grad_f(x(k + 1), y(k + 1));
        H = hess_f(x(k + 1), y(k + 1));
